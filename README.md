...
```
$captcha = new ApiCaptcha($config);

$id = '46463498765'; // 认证编号 id
...
[
    ...
    verify_code => 校验码
    verify_time => 验证码创建时间
    verify_img => 图像 base64 [ data:image/png;base64,{$verify_img} 放入 img 的 src 就可以显示图片]
]
```
...
$verify_id = $verify_id; // 认证编号 id
$user_verify_code = $verify_code; // 用户输入的验证码
        
$captcha = new ApiCaptcha($config);
// step 1 根据请求的验证码ID,从缓存中获取 原始 验证码的值
$captcha_key = $captcha->authcode($verify_id);

// 系统缓存在服务器的校验码
$verify_code = $verify_code; 

// step 2 对比用户输入验证码和原始验证码
$flag = $captcha->check($user_verify_code, $verify_code);
```
